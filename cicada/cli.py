...
from cicada.commands import wait
...
class Cicada():
    ...
    def upsert_schedule():
        ...
        parser.add_argument('--schedule_id', type=str, required=True,
            help='Id of the schedule')
        parser.add_argument('--schedule_description', type=str,
            help='Schedule description and comments')
        parser.add_argument('--server_id', type=int,
            help='Id of the server where the schedule will run')
        parser.add_argument('--schedule_order', type=int,
            help='run order for the schedule. lowest is first. is_async jobs will be execute in parallel')
        parser.add_argument('--is_async', type=str,
            help='0=disabled 1=Enabled | is_async jobs execute in parallel')
        parser.add_argument('--is_enabled', type=str,
            help='0=Disabled 1=Enabled')
        parser.add_argument('--adhoc_execute', type=str,
            help='0=Disabled 1=Enabled | Execute at next minute, regardless of other settings')
        parser.add_argument('--abort_running', type=str,
            help='0=Disabled 1=Enabled | If the job is running, it will be terminated as soon as possible')
        parser.add_argument('--interval_mask', type=str,
            help='When to execute the command | unix crontab (minute hour dom month dow)')
        parser.add_argument('--first_run_date', type=str,
            help='The schedule will not execute before this datetime')
        parser.add_argument('--last_run_date', type=str,
            help='The schedule will not execute after this datetime')
        parser.add_argument('--exec_command', type=str,
            help='Command to execute')
        parser.add_argument('--parameters', type=str,
            help='Parameters for exec_command')
        parser.add_argument('--adhoc_parameters', type=str,
            help='If specified, will override parameters for one run')
        parser.add_argument('--schedule_group_id', type=int,
    ...
    def spread_schedules():
        ...
        parser.add_argument('--from_server_ids', type=str, required=True,
            help='List of source server_ids to collect schedules from')
        parser.add_argument('--to_server_ids', type=str, required=True,
    ...
    def archive_schedule_log():
        ...
        parser.add_argument('--days_to_keep', type=int, required=True,
    ...
    def wait():
        ...
        parser = argparse.ArgumentParser(
            ...
            add_help=True,
            prog=inspect.stack()[0][3],
            description='Wait. Just wait.')
        parser.add_argument('--seconds', type=float, required=True)
        # now that we're inside a subcommand, ignore the first TWO args
        args = parser.parse_args(sys.argv[2:])
        wait.main(args.seconds)
