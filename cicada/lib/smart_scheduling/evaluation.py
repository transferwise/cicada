...
def evaluate_usage_and_peak(start_times: Sequence[int], schedules: Sequence[Schedule]):
    ...
    for i, schedule in enumerate(schedules):
        ...
        if start_times[i] >= freq:
            raise ValueError(
                f"Start time should be the earliest it can be for schedule: {schedule} with start time {start_times[i]} exceeds frequency {freq}"
            )
        if freq <= 0:
            raise ValueError(
                f"Unsupported frequency {freq} for schedule {schedule} {schedule.interval_mask} should have been labelled unsupported and caught earlier."
            )
