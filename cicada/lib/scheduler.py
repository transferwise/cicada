...
def generate_full_command(schedule_id: str, dbname = None):
...
def update_schedule_details(db_cur, schedule_details):
    ...
    if schedule_details['exec_command'] is not None:
        sqlquery = sqlquery + " ,exec_command = '" + str(schedule_details['exec_command']) + "'"
    if schedule_details['parameters'] is not None:
        sqlquery = sqlquery + " ,parameters = '" + str(schedule_details['parameters']) + "'"
    if schedule_details['adhoc_parameters'] is not None:
        sqlquery = sqlquery + " ,adhoc_parameters = '" + str(schedule_details['adhoc_parameters']) + "'"
    if schedule_details['schedule_group_id'] is not None:
    ...
    * * * * *
    ...
    for row in cur_schedules.fetchall():
        ...
        if not croniter.is_valid(interval_mask):
            utils.send_slack_message(f":warning:  *WARNING* invalid interval_mask on schedule_id *'{schedule_id}'*",
