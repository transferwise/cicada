...
def _get_schedules_per_server(server_id, db_cur=None):
    ...
    if not schedule_ids:
        raise ValueError(
            f"No schedules found for server_id {server_id}. Check if server_id exists and has schedules assigned."
        )
    ...
    for schedule_id in schedule_ids:
        ...
        try:
            ...
            if not schedule.is_regular_schedule():
                print(
                    f"Skipping irregular schedule {schedule.schedule_id} with cron expression {schedule.interval_mask}"
                )
            else:
...
def _update_schedule_cron(schedule: Schedule):
    ...
    if frequency == 1440:  # For daily schedules, we can shift within the hour
        ...
        if not croniter.is_valid(schedule.smart_interval_mask):
            raise ValueError(
                f"Invalid cron expression generated: {schedule.smart_interval_mask} for schedule {schedule.schedule_id}"
            )
        return
    elif frequency == 60:  # For hourly schedules, we can shift within the hour
        if start_time_mins >= frequency:
            raise ValueError(
                f"Shift {start_time_mins} cannot be greater than or equal to frequency {frequency} for schedule {schedule.schedule_id}"
            )
...
def main(server_id=None, dbname=None, ga_config=None):
    if server_id and type(server_id) != int:
        raise TypeError(f"server_id should be int not {type(server_id)}")
        ...
        try:
            ...
            if peak_usage < initial_fitness:
                try:
                    ...
                    scheduler.snapshot_schedules(
                        db_cur, server_id=server_id, computed_usage=peak_usage, reason="Smart Schedule Optimization"
                    )
                ...
                print(
                    f"No improvement found for server_id {server_id}. Current peak usage: {initial_fitness}, Optimized peak usage: {peak_usage}. No schedule updates will be made."
                )
                print("--------------------------------------------------\n")
