...
${CICADA_HOME}/venv/bin/cicada upsert_schedule --schedule_id=wait --is_enabled=1 --exec_command="${CICADA_HOME}/venv/bin/cicada wait" --parameters="--seconds=0.5" --interval_mask="* * * * *"
${CICADA_HOME}/venv/bin/cicada upsert_schedule --schedule_id=long_wait --is_enabled=1 --exec_command="${CICADA_HOME}/venv/bin/cicada wait" --parameters="--seconds=600" --interval_mask="* * * * *"
${CICADA_HOME}/venv/bin/cicada upsert_schedule --schedule_id=missing_exec --is_enabled=1 --exec_command="death.exe" --interval_mask="* * * * *"
${CICADA_HOME}/venv/bin/cicada upsert_schedule --schedule_id=missing_operand --is_enabled=1 --exec_command="${CICADA_HOME}/venv/bin/cicada wait" --interval_mask="* * * * *"
...
echo "Help                  : ${CICADA_HOME}/venv/bin/cicada -h"
echo "Register server       : ${CICADA_HOME}/venv/bin/cicada register_server"
echo "List server schedules : ${CICADA_HOME}/venv/bin/cicada list_server_schedules"
echo
