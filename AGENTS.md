...
- Every PR description must link directly to its latest CHANGELOG release section. Use an immutable commit permalink
  with the section anchor and the wording: `Refer to [VERSION CHANGELOG](URL) for full details.`
- Keep the PR **Changes** section concise and limited to positive user-visible or operational outcomes. Put
  implementation details, refactors, peripheral improvements, and test mechanics in the CHANGELOG, test summary, or
  validation section instead.
